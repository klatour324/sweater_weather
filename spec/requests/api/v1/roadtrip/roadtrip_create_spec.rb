...
RSpec.describe 'Road Trip Create' do
  ...
  describe 'happy path' do
    it 'creates a new roadtrip' do
      ...
      expect(result[:data][:type]).to have_key(:id)
      expect(result[:data][:type][:id]).to eq(nil)
      expect(result[:data]).to have_key(:attributes)
      expect(result[:data][:attributes]).to be_a(Hash)
      expect(result[:data][:attributes].key.count).to eq(4)
      ...
      expect(result[:data][:attributes]).to have_key(:travel)
      expect(result[:data][:attributes][:travel]).to be_a(String)
