...
RSpec.describe 'Forecast Index Page' do
  describe 'happy path' do
    it 'sends the current forecast for the given valid location' do
      VCR.use_cassette('forecast_for_location') do
        location = 'denver,co'
        ...
        expect(result[:data][:attributes][:current_weather][:temperature]).to be_a(Float).or be_an(Integer)
    ...
    it 'sends the hourly forecast for the given valid location' do
      VCR.use_cassette('hourly_forecast_for_location') do
        location = 'denver,co'
        get "/api/v1/forecast?location=#{location}"
  ...
  describe 'sad path' do
    ...
    it 'returns an error response if the location does not match anything' do
      VCR.use_cassette('crazy_location') do
        location = 'lkjsadflkjaiain932948'
        get "/api/v1/forecast?location=#{location}"
